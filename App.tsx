...
}> = ({ user, subscriptionStatus, subscriptionPlan, allSubjects, allLessons, allLiveClasses, lessonCompletions, activeLiveClass, onSelectSubject, onJoinLiveClass, onPayForLessons, onWatchLesson, onNavigateToBookstore }) => {
...
const TeacherDashboard: React.FC<{ user: User }> = ({ user }) => (
    <div className="p-4 text-slate-800 dark:text-slate-100 animate-fade-in-up">
        <h1 className="text-2xl font-bold">Teacher Dashboard</h1>
        <p>Welcome, {user.name}. This area is under construction.</p>
        <p>Features like course management, student analytics, and content creation will be available here soon.</p>
    </div>
);

const OwnerDashboard: React.FC<{ user: User; allStudents: User[]; payments: PaymentRecord[]; }> = ({ user, allStudents, payments }) => {
    const sortedStudents = [...allStudents].filter(u => u.role === Role.Student).sort((a, b) => a.name.localeCompare(b.name));
    ...
    return (
        <div className="p-4 text-slate-800 dark:text-slate-100 animate-fade-in-up space-y-6">
            <div>
                ...
                <p>Welcome, {user.name}. Oversee platform activity here.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <StatCard icon={<UsersIcon />} title="Total Students" value={sortedStudents.length} gradient="text-white bg-gradient-to-br from-blue-500 to-indigo-600" />
                ...
                <StatCard icon={<RssIcon />} title="Active Plans" value={sortedStudents.filter(s => getSubscriptionStatus(s).status === 'Active').length} gradient="text-white bg-gradient-to-br from-purple-500 to-violet-600" />
            </div>
            
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                 {/* Student Subscriptions Table */}
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm">
                    <h2 className="text-xl font-bold mb-4">Student Subscriptions</h2>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {sortedStudents.map(student => {
                                    const { status, plan } = getSubscriptionStatus(student);
                                    let statusColor = 'text-slate-500';
                                    if (status === 'Active') statusColor = 'text-green-500';
                                    if (status === 'Expired') statusColor = 'text-red-500';

                                    return (
                                        <tr key={student.id} className="border-b dark:border-slate-700">
                                            <td className="p-2 font-medium">{student.name}</td>
                                            <td className={`p-2 font-semibold ${statusColor}`}>{plan}</td>
                                            <td className="p-2">{status === 'Active' ? student.subscription?.endDate.toLocaleDateString() : 'N/A'}</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
                        ...
                        {sortedPayments.map(p => (
                            <div key={p.id} className="flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{p.date.toLocaleString()}</p>
                                </div>
                                <div className="text-right">
                                    <p className="font-bold text-green-600 dark:text-green-400">+K{p.amount.toLocaleString()}</p>
                                    <p className="text-xs text-slate-500">{p.plan} via {p.method}</p>
                                </div>
                            </div>
                        ))}
                     </div>
...
const SubjectDetailView: React.FC<{
    ...
    onWatchLesson: (lesson: VideoLesson) => void;
}> = ({ user, subject, lessons, completions, onWatchLesson }) => {
        ...
        return (
            <>
                ...
                <AuthScreen 
                    ...
                    onApply={() => addToast('Application form is not implemented yet.', 'info')}
                />
            </>
        );
    }
        ...
        switch (currentUser.role) {
            case Role.Student:
                ...
                return <StudentDashboard 
                            ...
                            activeLiveClass={null} // Placeholder
            ...
            case Role.Owner:
                return <OwnerDashboard user={currentUser} allStudents={users} payments={payments} />;
            default:
